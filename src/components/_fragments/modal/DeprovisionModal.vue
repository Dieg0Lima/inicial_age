    ...
    <div class="modal-content w-4/12">
      <div class="modal-header flex flex-row justify-between space-x-8">
        <div v-if="currentPage === 'form'" class="flex flex-col space-y-4">
          <h1 class="font-bold text-xl">Formulário de Desprovisionamento</h1>
          <span
          ...
          >
            <div class="flex flex-row w-full space-x-4">
              <div class="w-full flex flex-col space-y-2">
                <span class="w-full font-semibold">Slot</span>
                <input
                  class="p-2 border w-full border-solid border-slate-200 rounded-md"
                  type="text"
                  v-model="formDeprovision.slot"
                  placeholder="Slot"
                  required
                />
              </div>
              <div class="w-full flex flex-col space-y-2">
                <span class="w-full font-semibold">Pon</span>

                <input
                  class="p-2 border w-full border-solid border-slate-200 rounded-md"
                  type="text"
                  v-model="formDeprovision.pon"
                  placeholder="Pon"
                  required
                />
              </div>
              <div class="w-full flex flex-col space-y-2">
                <span class="w-full font-semibold">Porta</span>

                <input
                  class="p-2 border w-full border-solid border-slate-200 rounded-md"
                  type="text"
                  v-model="formDeprovision.port"
                  placeholder="Porta"
                  required
                />
              </div>
            ...
            <div class="flex flex-row w-full space-x-4">
              <div class="w-full flex flex-col space-y-2">
                ...
                <input
                  class="p-2 border w-full border-solid border-slate-200 rounded-md"
                  type="text"
                  v-model="formDeprovision.sernum"
                  placeholder="Número de Serial"
                  required
                />
              </div>
              <div class="w-full flex flex-col space-y-2">
                <span class="w-full font-semibold">OLT</span>
                <input
                  class="p-2 border w-full border-solid border-slate-200 rounded-md"
                  type="text"
                  :value="localAccessPoint"
                  @input="updateAccessPoint($event.target.value)"
                  placeholder="Número de Serial"
                  required
                  disabled
                />
...
import { useClientDeprovisionStore } from "@/stores/clientDeprovisionStore";
...
const formDeprovision = reactive({
  slot: "",
  pon: "",
  port: "",
  sernum: "",
...
const clientDeprovisionStore = useClientDeprovisionStore();
const localAccessPoint = ref(
  props.connection ? props.connection.access_point : ""
);
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      formDeprovision.sernum = newVal.equipment_serial_number || "";
      formDeprovision.slot = newVal.slot_olt || "";
      formDeprovision.pon = newVal.port_olt || "";
      formDeprovision.port = newVal.olt_id || "";
    }
  },
...
const finalSubmit = () => {
  ...
  const submissionData = {
    sernum: formDeprovision.sernum,
    slot: formDeprovision.slot,
    pon: formDeprovision.pon,
    port: formDeprovision.port,
    olt_id: props.connection.access_point_id || "",
  };

  clientDeprovisionStore

    .submitDeprovision(submissionData)
    .then(() => {
      loading.value = false;
