    ...
    <div class="modal-content w-4/12">
      <div class="modal-header flex flex-row justify-between space-x-8">
        <div v-if="currentPage === 'form'" class="flex flex-col space-y-4">
          <h1 class="font-bold text-xl">Formulário de Reinicialização</h1>
          <span
...
import { useClientRebootStore } from "@/stores/clientRebootStore";
...
const clientRebootStore = useClientRebootStore();
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      formReboot.sernum = newVal.equipment_serial_number || "";
      formReboot.slot = newVal.slot_olt || "";
      formReboot.pon = newVal.port_olt || "";
      formReboot.port = newVal.olt_id || "";
    }
  },
...
const finalSubmit = () => {
  ...
  const submissionData = {
    sernum: formReboot.sernum,
    slot: formReboot.slot,
    pon: formReboot.pon,
    port: formReboot.port,
    olt_id: props.connection.access_point_id || "",
  };

  clientRebootStore
    .submitReboot(submissionData)
    .then(() => {
      loading.value = false;
